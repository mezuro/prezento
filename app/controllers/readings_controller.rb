...
class ReadingsController < ApplicationController
  ...
  def update
    ...
    respond_to do |format|
      if @reading.update(reading_params)
        format.html { redirect_to(reading_group_path(params[:reading_group_id].to_i), notice: t('reading_sucessfully_update')) }
  ...
  def create_and_redir(format)
    if @reading.save
      format.html { redirect_to reading_group_path(@reading.group_id), notice: t('reading_successfully_created') }
    else
