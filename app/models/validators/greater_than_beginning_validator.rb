class GreaterThanBeginningValidator < ActiveModel::EachValidator
  def validate_each(record, attribute, value)
    if record.beginning.is_a?(String) || value.is_a?(String) #TOOO This will be useless when we start representing INF as ruby Infinity with the new Kalibro configuration application.
