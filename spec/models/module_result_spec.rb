...
describe ModuleResult, :type => :model do
  describe 'methods' do
    ...
    describe 'metric_history' do
      ...
      let(:metric_result) { FactoryGirl.build(:metric_result, configuration: FactoryGirl.build(:metric_configuration_with_snapshot)) }
      let(:processing) {FactoryGirl.build(:processing)}
      let(:repository) {FactoryGirl.build(:repository)}

      before :each do
        subject.expects(:processing).returns(processing)
        metric_result.expects(:metric_configuration).returns(metric_result.configuration)
      ...
      it 'should return the history for the given metric name' do
        expect(subject.metric_history(metric_result.configuration.metric_snapshot.name)).to eq({date_module_result.date => metric_result.value})
      end
    end
  end
end
