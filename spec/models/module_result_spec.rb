...
describe ModuleResult, :type => :model do
  describe 'methods' do
    ...
    describe 'metric_history' do
      ...
      let!(:metric_result) { FactoryGirl.build(:metric_result, metric_configuration: FactoryGirl.build(:another_metric_configuration)) }
      ...
      it 'should return the history for the given metric name' do
        expect(subject.metric_history(metric_result.metric_configuration.metric.name)).to eq({date_module_result.date => metric_result.value})
      end
    end
  end
end
